...
from utils import window, skip, trocarTester, calculate_force
...
for i in range(6):
    ...
    model = trocarTester(folder, network, window, skip, [i], in_joints, batch_size, device, path, is_rnn=is_rnn)
    ...
    if all_pred is None:
        all_pred = pred
        diff_uncorrected = torque - fs_torque
...
np.savetxt(path + '/uncorrected_forces.csv', uncorrected_forces.numpy()) 
...
np.savetxt(path + '/corrected_forces.csv', corrected_forces.numpy()) 

all_pred = torch.cat((time.unsqueeze(1), all_pred), axis=1)
np.savetxt(path + '/pred.csv', all_pred.numpy())
print('saved pred')
