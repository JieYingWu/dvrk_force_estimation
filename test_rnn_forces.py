...
for i in range(6):
    ...
    model = trocarTester(folder, network, window, skip, [i], in_joints, batch_size, device, fs_network, path, is_rnn=True, use_jaw=False)
    ...
    if all_pred is None:
        all_pred = pred
        diff_uncorrected = torque - fs_pred[:,-1,:]
        diff_corrected = torque - pred - fs_pred[:,-1,:]
    else:
        all_pred = torch.cat((all_pred, pred), axis=1)
        diff_uncorrected = torch.cat((diff_uncorrected, torque-fs_pred[:,-1,:]), axis=1)
        diff_corrected = torch.cat((diff_corrected, torque-pred-fs_pred[:,-1,:]), axis=1)
