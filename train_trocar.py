...
is_seal = int(sys.argv[1])
if is_seal:
    folder = 'ff/trocar'
else:
    folder = 'ff/trocar_no_cannula'
...
for num in ['60', '120', '180', '240', '300', '360']:
    ...
    train_dataset = indirectTrocarDataset(train_path, WINDOW, SKIP, in_joints, num=num, is_seal=is_seal, filter_signal=f)
    val_dataset = indirectTrocarDataset(val_path, WINDOW, SKIP, in_joints, num=num, is_seal=is_seal, filter_signal=f)
    ...
    for e in range(epoch, epochs + 1):
        ...
        for i, (position, velocity, torque, jacobian, time, fs_pred) in enumerate(train_loader):
            ...
            for j in range(JOINTS):
                ...
                pred = networks[j](posvel) + fs_pred[:,[(j+1)*WINDOW-1]]
        ...
        if e % validate_each == 0:
            ...
            for i, (position, velocity, torque, jacobian, time, fs_pred) in enumerate(val_loader):
                position = position.to(device)
                velocity = velocity.to(device)
                fs_pred = fs_pred.to(device)
                torque = torque.to(device)
                ...
                for j in range(JOINTS):
                    posvel = torch.cat((position, velocity, fs_pred[:,[[(j+1)*WINDOW-1]]]), axis=1).contiguous()
