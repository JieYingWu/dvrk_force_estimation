...
preprocess = 'filtered_torque_360s' #sys.argv[3]
net = 'ff' #sys.argv[4]
seal = sys.argv[1]
is_seal = seal == 'seal'
print('is_seal value: ', is_seal)
...
def main():
    for exp in ['exp0', 'exp1', 'exp2', 'exp3', 'exp4']:
        ...
        dataset = indirectTrocarTestDataset(path, window, skip, in_joints, is_seal=is_seal, filter_signal=True)
        ...
        np.savetxt(results_path + '/uncorrected_forces_' + seal + '_' + net + '_' + preprocess + '.csv', all_fs_force.numpy()) 

        all_force = utils.calculate_force(all_jacobian, all_diff)
        all_force = torch.cat((all_time, all_force), axis=1)
        np.savetxt(results_path + '/corrected_forces_' + seal + '_' +net + '_' + preprocess + '.csv', all_force.numpy())
